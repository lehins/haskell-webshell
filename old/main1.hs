...
main :: IO ()
main = do
  let child sin sout serr = do
        dupTo sin stdInput
        dupTo sout stdOutput
        dupTo serr stdError
  (mfd, sfd) <- openPseudoTerminal

  (stdinread, stdinwrite) <- createPipe
  (stdoutread, stdoutwrite) <- createPipe
  (stderrread, stderrwrite) <- createPipe
  liftIO $ child stdinread stdoutwrite stderrwrite
  hin <- liftIO $ fdToHandle stdinwrite
  hout <- liftIO $ fdToHandle stdoutread
  herr <- liftIO $ fdToHandle stderrread
  liftIO $ hSetBuffering hin NoBuffering
  mvhin <- newMVar hin
  mvhout <- newMVar hout
  mvherr <- newMVar herr
  pid <- forkProcess $ simpleHTTP nullConf (kash (KashState mvhin mvhout mvherr))
...
communicateBack att st@(KashState _ mvhout mvherr) = do
