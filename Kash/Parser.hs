...
import Data.Attoparsec hiding (inClass, satisfy)
...
pPm def s = fmap appendC (many (pPs Nothing ';')) <*> (pPs def s) where
  appendC ls c = ls++[c]
...
csiFunc = choice $ [
  ...
  CUP <$> ((pure :: Int -> Parser Ps) 1) <*> (((pure ::Int -> Parser Ps) 1) <* char 'H'),
  CHT <$> (pPs (Just 1) 'I'),
  ED  <$> (pPs (Just 0) 'J'),
  ...
  SU  <$> (pPs (Just 1) 'S'),
  SD  <$> (pPs (Just 1) 'T'),
  MouseTrackInit <$> (pPs Nothing ';') <*> (pPs Nothing ';') <*>
    (pPs Nothing ';') <*> (pPs Nothing ';') <*> (pPs Nothing 'T'),
  ...
  DECCARA <$> (pPs Nothing ';') <*> (pPs Nothing ';') <*>
    (pPs Nothing ';') <*> (pPs Nothing ';') <*> ((pPs Nothing '$') <* char 'r'),
  DECSLRM <$> (pPs Nothing ';') <*> (pPs Nothing 's'),
  CURSave <$ (char 's'),
  DECSaveM <$> (char '?' *> (pPm Nothing 's')),
  WindowSet <$> (pPs Nothing ';') <*> (pPs (Just $ -1) ';') <*> (pPs (Just $ -1) 't'),
  DECRARA <$> (pPs Nothing ';') <*> (pPs Nothing ';') <*>
    (pPs Nothing ';') <*> (pPs Nothing ';') <*> ((pPs Nothing '$') <* char 't'),
  ...
  DECCRA <$> (pPs Nothing ';') <*> (pPs Nothing ';') <*> (pPs Nothing ';') <*> 
    (pPs Nothing ';') <*> (pPs Nothing ';') <*> (pPs Nothing ';') <*> 
    (pPs Nothing ';') <*> ((pPs Nothing '$') <* char 'v'),
  ...
  DECRQCRA <$> (pPs Nothing ';') <*> (pPs Nothing ';') <*> (pPs Nothing ';') <*> 
    (pPs Nothing ';') <*> (pPs Nothing ';') <*> ((pPs Nothing '*') <* char 'y'),
  DECFRA <$> (pPs Nothing ';') <*> (pPs Nothing ';') <*> (pPs Nothing ';') <*> 
    (pPs Nothing ';') <*> ((pPs Nothing '$') <* char 'x'),
...
data KSeq = KString B.ByteString | KControl CtrlSeq | KUnknown Char deriving Show
...
tParse s = parseOnly kSeq s


main = do 
  testfile <- openFile "vimtest.txt" ReadMode
  testcontent <- B.hGetContents testfile
  C8.putStrLn testcontent
  let result = parseOnly kSeq testcontent
  case result of
